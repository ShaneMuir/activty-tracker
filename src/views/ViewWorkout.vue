  ...
  <div class="max-w-screen-sm mx-auto px-4 py-10">
    ...
    <div v-if="dataLoaded">
      ...
      bg-light-grey relative">
        <div v-if="user">
          <div v-if="user.id === data.belongsTo" class="flex absolute left-2 top-2 gap-x-2">
            <div class="h-7 w-7 rounded-full flex justify-center items-center cursor-pointer
        bg-at-blue shadow-lg"
                 @click="editMode">
        ...
        <img v-else
            class="h-24 w-auto"
            src="@/assets/images/dumbell-blue.svg"
            alt=""/>

        <span class="mt-6 py-1.5 px-5 text-xs text-white bg-at-blue
        ...
        <div class="w-full mt-6">
          ...
          <h1 v-else class="text-at-blue text-2xl text-center">
            {{ data.workoutName }}
          </h1>
        </div>
      </div>
      ...
      bg-light-grey shadow-md">
        ...
        <div v-if="data.workoutType === 'strength'" class="flex flex-col gap-y-4 w-full">
          <div class="flex flex-col gap-x-6 gap-y-2 relative sm:flex-row"
            ...
            <div class="flex flex-2 flex-col md:w-1/3">
              ...
              <input id="exercise-name" v-if="edit"
                  class="p-2 w-full text-gray-500 focus:outline-none" type="text"
                  v-model="item.exercise"/>
              <p v-else>{{ item.exercise }}</p>
            </div>
            <div class="flex flex-1 flex-col">
              <label for="sets" class="mb-1 text-sm text-at-blue">
                Sets
              </label>
              <input v-if="edit" id="sets"
                  class="p-2 w-full text-gray-500 focus:outline-none" type="text" v-model="item.sets"/>
              <p v-else>{{ item.sets }}</p>
            </div>
            <div class="flex flex-1 flex-col">
              <label for="reps" class="mb-1 text-sm text-at-blue">
                Reps
              </label>
            ...
            <div class="flex flex-1 flex-col">
              <label for="weight" class="mb-1 text-sm text-at-blue">
                Weight (LB's)
              </label>
              <input v-if="edit" id="weight"
                  class="p-2 w-full text-gray-500 focus:outline-none" type="text"
                  v-model="item.weight"/>
              <p v-else>{{ item.weight }}</p>
            </div>
            <img v-if="edit"
                @click="deleteExercise(item.id)" class="absolute h-4 w-auto -left-5 cursor-pointer"
                src="@/assets/images/bin.svg" alt=""/>
          </div>
          <button v-if="edit" @click="addExercise" type="button"
              class="py-2 px-6 rounded-sm self-start text-sm text-white
            bg-at-blue duration-200 border-solid border-2 border-transparent
            hover:border-at-blue hover:bg-white hover:text-at-blue">
            Add Exercise
          </button>
        </div>
        ...
        <div v-else class="flex flex-col gap-y-4 w-full">
          <div class="flex flex-col gap-x-6 gap-y-2 relative sm:flex-row"
              v-for="(item, index) in data.exercises" :key="index">
            <div class="flex flex-2 flex-col md:w-1/3">
              <label for="cardioType" class="mb-1 text-sm text-at-blue">
                Type
              </label>
              <select id="cardioType"
                  v-if="edit" class="p-2 w-full text-gray-500 focus:outline-none" type="text"
            ...
            <div class="flex flex-1 flex-col">
              <label for="duration" class="mb-1 text-sm text-at-blue">
                Duration
              </label>
            ...
            <div class="flex flex-1 flex-col">
              <label for="pace" class="mb-1 text-sm text-at-blue">
                Pace
              </label>
          ...
          <button @click="addExercise" v-if="edit" type="button"
              class="py-2 px-6 rounded-sm self-start text-sm text-white
            bg-at-blue duration-200 border-solid border-2 border-transparent
            hover:border-at-blue hover:bg-white hover:text-at-blue">
            Add Exercise
      ...
      <!-- Update -->
      <button v-if="edit" @click="update" type="button"
